...
function updateChart() {
    chart.data.datasets[0].label = labels[currentMetric];
    chart.data.datasets[0].data = data[currentBurger][currentMetric];
    chart.update();
    document.getElementById("chartTitle").textContent = labels[currentMetric] + " 比較";
}
...
document.querySelectorAll(".burger-btn").forEach(button => {
    button.addEventListener("click", () => {
        currentBurger = button.dataset.burger;
